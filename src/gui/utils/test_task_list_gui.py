...
def test_init_validation():
    with pytest.raises(TypeError, match="Not a String or NoneType."):
        TaskList(123)  # Not a string
    with pytest.raises(AttributeError, match="Incorrect extension. Must be '.json'."):
        TaskList("test.jsn")  # Typo in file extension
    file = "doesnt_exist.json"
    with pytest.raises(FileNotFoundError, match=r"\[Errno 2\] No such file or directory: 'doesnt_exist\.json'"):
        TaskList(file)
